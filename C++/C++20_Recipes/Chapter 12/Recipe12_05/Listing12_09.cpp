...
//
// Name: Listing12_09.cpp
// Author: crdrisko
// Date: 10/12/2020-17:23:10
// Description: A simple quiz protocol client
...
int main(int argc, char* argv[])
{
    ...
    std::string address {"192.168.0.30"};
    ...
    if (connectionResult != -1)
    {
        std::stringstream output {"QUESTION"};
        mySocket.Send(std::move(output));

        std::stringstream input {mySocket.Receive()};
        if (input.rdbuf()->in_avail() > 0)
        {
            std::string question;
            std::getline(input, question, '\0');
            input.clear();
