...
|* Name: Ex5_14.cpp                                          *|
|* Author: crdrisko                                          *|
|* Description: Using an array<T,N> to create Body Mass      *|
|*  Index (BMI) table. BMI = weight/(height*height)          *|
|*  weight in kilograms, height in meters                    *|
|*                                                           *|
|*                                                           *|
|* Complile Command: g++ Ex5_14.cpp -o Ex5_14.out -std=c++17 *|
\*************************************************************/
...
int main()
{
    ...
    const unsigned max_wt {250};                // Maximum weight in table (in pounds)
    const unsigned wt_step {10};
    const size_t wt_count {1 + (max_wt - min_wt) / wt_step};

    const unsigned min_ht {48};                 // Minimum height in table (in inches)
    const unsigned max_ht {84};                 // Maximum height in table (in inches)
